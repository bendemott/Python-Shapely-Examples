...
def main():
	...
	for idx, point in enumerate(shapePoints):
		...
		for index in sortedIdx:
			...
			matrixEdges.append([index, distance, pointObj.x, pointObj.
		# Take all of the connections 
		lineMatrices.append(matrixEdges)

		# Setup bla
		print 'asdf'
		dp=shortestPath.shortestPath()

		# Randomely select the start and end vertexes based on a random index.
		startIdx = random.randint(0, numberOfPoints-1)
		endIdx = random.randint(0, numberOfPoints-1)
		...
		for idx, edges in enumerate(lineMatrices):
			...
			if(idx == endIdx):
				dijkstraPath.setEndVertex(iVertex)

		# Perform the calculation!
		distance, vertexPath = dijkstraPath.calculate()
